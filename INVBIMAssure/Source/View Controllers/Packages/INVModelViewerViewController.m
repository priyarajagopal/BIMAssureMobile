...
- (void)setupScene
{
    ...
    GLKVector3 lightPostions[6] = {{1000, 0, 0}, {0, 1000, 0}, {0, 0, 1000}, {-1000, 0, 0}, {0, -1000, 0}, {0, 0, -1000}};
...
- (void)update
{
    ...
    */
    ...
    _camera.projectionTransform = GLKMatrix4Multiply(GLKMatrix4MakePerspective(55, aspect, 1, 10000),
        GLKMatrix4MakeLookAt(cameraPosition.x, cameraPosition.y, cameraPosition.z, projectedPosition.x, projectedPosition.y,
                                                         projectedPosition.z, 0, 0, -1));
                  ...
                  shouldStop:(BOOL *)stop
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        INVLogDebug(
            @"Currently: %10lu verts, %10lu tris.", (unsigned long) self->_vertexCount, (unsigned long) self->_triangleCount);
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touches.count == 1) {
        ...
        rotationMatrix = GLKMatrix4Multiply(
            GLKMatrix4Translate(
                GLKMatrix4Rotate(GLKMatrix4MakeTranslation(-camera.x, camera.y, -camera.z), 0.05, -changedY, 0, -changedX),
    ...
    if (touches.count == 2) {
        ...
        float distance =
            GLKVector2Distance(GLKVector2Make(touch1Point.x, touch1Point.y), GLKVector2Make(touch2Point.x, touch2Point.y));

        float lastDistance = GLKVector2Distance(
            GLKVector2Make(touch1LastPoint.x, touch1LastPoint.y), GLKVector2Make(touch2LastPoint.x, touch2LastPoint.y));
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touch.tapCount == 2) {
        ...
        GLKVector4 rayEyeCoordinates = GLKMatrix4MultiplyVector4(GLKMatrix4Invert(_camera.projectionTransform, NULL),
        rayClipCoordinates);
        rayEyeCoordinates.z = 1;
        rayEyeCoordinates.w = 0;

        GLKVector4 rayWorldCoordinates = GLKMatrix4MultiplyVector4(GLKMatrix4Invert(_camera.modelViewTransform, NULL),
        rayEyeCoordinates);
        GLKVector3 rayDirection = GLKVector3Normalize(GLKVector3Make(rayWorldCoordinates.x, rayWorldCoordinates.y,
        rayWorldCoordinates.z));
