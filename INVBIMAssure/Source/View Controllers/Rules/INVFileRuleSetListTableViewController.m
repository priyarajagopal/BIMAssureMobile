                 ...
                 if (!error) {
                     ...
                     UIAlertController *errController = [[UIAlertController alloc]
                         initWithErrorMessage:
                             [NSString stringWithFormat:NSLocalizedString(@"ERROR_RULESET_MEMBERSHIP_LOAD", nil), error.code]];
                         ...
                         if (!error) {
                             ...
                             UIAlertController *errController = [[UIAlertController alloc]
                                 initWithErrorMessage:[NSString stringWithFormat:NSLocalizedString(
                                                                                     @"ERROR_RULESET_MEMBERSHIP_LOAD", nil),
                                                                error.code]];
                             [self presentViewController:errController animated:YES completion:nil];
                         }
                     }];
...
{
    NSSet *currentRuleSetsForPkgMaster =
        [self.globalDataManager.invServerClient.rulesManager ruleSetIdsForPkgMaster:self.fileId];
...
{
    NSMutableSet *currentRuleSetsForPkgMaster =
        [[self.globalDataManager.invServerClient.rulesManager ruleSetIdsForPkgMaster:self.fileId] mutableCopy];
    NSMutableSet *updatedRuleSetIds = [[NSMutableSet alloc] initWithCapacity:0];
    [self.ruleSets enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        INVRuleSet *ruleSet = obj;
        [updatedRuleSetIds addObject:ruleSet.ruleSetId];
...
- (INVGenericTableViewDataSource *)ruleSetsDataSource
{
    if (!_ruleSetsDataSource) {
        _ruleSetsDataSource = [[INVGenericTableViewDataSource alloc] initWithDataArray:self.ruleSets
                                                                            forSection:SECTION_RULESETLIST
                                                                          forTableView:self.tableView];

        INV_CellConfigurationBlock cellConfigurationBlock =
            ^(INVGeneralAddRemoveTableViewCell *cell, INVRuleSet *ruleSet, NSIndexPath *indexPath) {
        ...
        [_ruleSetsDataSource registerCellWithIdentifierForAllIndexPaths:@"ProjectFileCell"
...
- (void)updateRuleSetsFromServer
{
    ...
    INVRuleSetMutableArray ruleSetsAssociatedWithFile =
        [[self.rulesManager ruleSetsForIds:[rulesetIdsInFile allObjects]] mutableCopy];
    if (self.showRuleSetsForFileId) {
