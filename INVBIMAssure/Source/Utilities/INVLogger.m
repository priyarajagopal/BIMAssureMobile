...
static inline void writeLogfilePreamble(FILE *logFile, NSDateFormatter *dateFormat) {
    NSString *logFilePreamble = [NSString stringWithFormat:@"\n\n---- BEGIN RUN AT %@ ----\n\n", [dateFormat stringFromDate:[NSDate date]]];
...
void _INVLog(INVLogLevel level, const char *func, const char *file, int line, NSString *format, ...) {
    if (level < _INVMinimumLogLevel) return;
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (_INVFileLoggingEnabled) {
            NSString *logPath = [[NSTemporaryDirectory() stringByAppendingPathComponent:[[NSProcessInfo processInfo] processName]] stringByAppendingPathExtension:@"log"];
