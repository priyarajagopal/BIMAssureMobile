...
static const GLKBBox GLKBBoxEmpty = {FLT_MAX, FLT_MAX, FLT_MAX, -FLT_MAX, -FLT_MAX, -FLT_MAX};
...
{
    return (GLKBBox){
        GLKVector3Minimum(left.min, right.min), GLKVector3Maximum(left.max, right.max),
    };
}

static inline GLKBBox GLKBBoxUnionVector3(GLKBBox left, GLKVector3 right)
{
    return (GLKBBox){GLKVector3Minimum(left.min, right), GLKVector3Maximum(left.max, right)};
}
...
{
    return (vector.x >= bbox.min.x && vector.x <= bbox.max.x) && (vector.y >= bbox.min.y && vector.y <= bbox.max.y);
}
