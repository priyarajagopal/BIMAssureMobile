...
- (void)awakeFromNib
{
    ...
    UITapGestureRecognizer *profTapGesture =
        [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onProfessionalViewTapped:)];
    [self.profView addGestureRecognizer:profTapGesture];
    [self.profView setUserInteractionEnabled:YES];

    UITapGestureRecognizer *teamTapGesture =
        [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onTeamViewTapped:)];
    [self.teamView addGestureRecognizer:teamTapGesture];
    [self.teamView setUserInteractionEnabled:YES];

    UITapGestureRecognizer *entTapGesture =
        [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onEnterpriseViewTapped:)];
...
- (void)addToView:(UIView *)view subscriptionDetails:(NSDictionary *)details
{
    ...
    [details enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        NSMutableAttributedString *detail =
            [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@:%@", heading, value]];
        ...
        UILabel *label =
            [[UILabel alloc] initWithFrame:CGRectMake(xOffset, yOffset, CGRectGetWidth(view.frame) - (xOffset * 2), 20)];
...
#warning Ideally we should have the subscription info fetched from server and the units should be localized probably
    self.professionalSubscriptionDetails = @{
        INV_SUBSCRIPTION_STORAGE : @"250 MB",
        INV_SUBSCRIPTION_ANALYSIS : @"500 MB",
        INV_SUBSCRIPTION_USERS : @"unlimited",
        INV_SUBSCRIPTION_TIER1RULES : @"unlimited",
        INV_SUBSCRIPTION_PRICING : @"$99/mo"
    };
    self.teamSubscriptionDetails = @{
        INV_SUBSCRIPTION_STORAGE : @"1 GB",
        INV_SUBSCRIPTION_ANALYSIS : @"2 GB",
        INV_SUBSCRIPTION_USERS : @"unlimited",
        INV_SUBSCRIPTION_TIER1RULES : @"unlimited",
        INV_SUBSCRIPTION_PRICING : @"$299/mo"
    };
    self.enterpriseSubscriptionDetails = @{
        INV_SUBSCRIPTION_STORAGE : @"4 GB",
        INV_SUBSCRIPTION_ANALYSIS : @"8 GB",
        INV_SUBSCRIPTION_USERS : @"unlimited",
        INV_SUBSCRIPTION_TIER1RULES : @"unlimited",
        INV_SUBSCRIPTION_PRICING : @"$999/mo"
    };
}
@end
